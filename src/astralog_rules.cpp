...
std::shared_ptr<Rule>
Rule::from_json(const json &json, std::vector<std::shared_ptr<Rule>> &rules) {
  ...
  if (type_str != "SIMPLE" && type_str != "STEP_DIFFERENCE" &&
      type_str != "STATEFUL" && type_str != "CORRELATION") {
    return nullptr;
  }
  RuleType type;
  switch (type_str[2]) {
  ...
  if (type == RuleType::CORRELATION) {
    ...
    std::vector<std::string> conditions =
        json["conditions"].get<std::vector<std::string>>();
    ...
    if (type == RuleType::SIMPLE) {
      ...
      StepDifferenceRule rule =
          StepDifferenceRule(id, threshold, priority, sensor_id, op);
      return std::make_shared<StepDifferenceRule>(rule);
    } else if (type == RuleType::STATEFUL) {
      if (!json.contains("consecutive_measurements") ||
          !json["consecutive_measurements"].is_number_integer()) {
        return nullptr;
      }
      int consecutive_measurements =
          json["consecutive_measurements"].get<int>();
      StatefulRule rule = StatefulRule(id, threshold, priority, sensor_id, op,
...
SimpleRule::SimpleRule(const std::string &id, double threshold,
...
EvaluationResult SimpleRule::evaluate(const std::string &sensor_id,
  ...
  if (op_ == '>') {
    return value > threshold_ ? EvaluationResult::ANOMALY
                              : EvaluationResult::NOMINAL;
  }
  return value < threshold_ ? EvaluationResult::ANOMALY
                            : EvaluationResult::NOMINAL;
}
...
StepDifferenceRule::StepDifferenceRule(const std::string &id, double threshold,
                                       std::optional<Priority> priority,
                                       const std::string &sensor_id, char op) {
  id_ = id;
  threshold_ = threshold;
...
std::optional<Priority> StepDifferenceRule::priority() const {
  return priority_;
}
...
EvaluationResult StepDifferenceRule::evaluate(const std::string &sensor_id,
  ...
  prev_value = value;
  if (op_ == '>') {
    return value > actual_threshold ? EvaluationResult::ANOMALY
                                    : EvaluationResult::NOMINAL;
  }
  return value < actual_threshold ? EvaluationResult::ANOMALY
                                  : EvaluationResult::NOMINAL;
...
StatefulRule::StatefulRule(const std::string &id, double threshold,
                           std::optional<Priority> priority,
                           const std::string &sensor_id, char op,
                           int consecutive_measurements) {
  id_ = id;
...
EvaluationResult StatefulRule::evaluate(const std::string &sensor_id,
  ...
  return count == consecutive_measurements_ ? EvaluationResult::ANOMALY
                                            : EvaluationResult::NOMINAL;
}

// CorrelationRule implementation
CorrelationRule::CorrelationRule(
    const std::string &id, std::optional<Priority> priority,
