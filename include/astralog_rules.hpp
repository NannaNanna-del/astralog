...
private:
  ...
  char op_; // '>' or '<'
  double prev_value =
      0.0; // TODO: need to initialize properly( for the first measurement)

public:
  StepDifferenceRrule(const std::string &id, double threshold,
                      Priority priority, const std::string &sensor_id,
                      char op) {
    id_ = id;
    threshold_ = threshold;
    priority_ = priority;
...
private:
  ...
  std::vector<std::shared_ptr<Rule>>
      parent_rules_; // TODO: initialization to be improved

public:
  CorrelationRule(const std::string &id, Priority priority,
...
// TODO: the requirement file said that priority is optional, maybe need a
// differente initialization for the rules
} // namespace astralog::rules
